...
def compute_metrics(hits: List[int], black_threshold: int = DEFAULT_BLACK_THRESHOLD) -> Dict[str, Any]:
    ...
    hit20_count = int(sum(hits[-20:])) if hits else 0
    hit10_count = int(sum(hits[-10:])) if hits else 0
    ...
    protect = (
        ...
        0.25 * r100 +
        0.20 * stab +
        0.20 * (1.0 - min(1.0, maxb / 25.0))
    ) * sample_factor
    protect = round(max(0.0, min(1.0, protect)), 4)

    # 做空分（越高越偏向过热回落）
    t2_fade = (1.0 - t2_next1) if t2_cnt > 0 else 0.5
    t3_fade = (1.0 - t3_next1) if t3_cnt > 0 else 0.5
    short_score = (
        0.35 * overheat +
        0.15 * max(0.0, r50 - r100) +
        0.15 * min(1.0, curr / 6.0) +
        0.20 * t2_fade +
        0.10 * t3_fade +
        0.05 * (1.0 - protect)
    )
    short_score = round(max(0.0, min(1.0, short_score)), 4)
